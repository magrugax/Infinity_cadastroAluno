...
def editarAluno() -> None:
    ...
    if opcao:
        ...
        limparCampos()
        atualizarTabela()
        messagebox.showinfo("Atualizado!","Dados alterados com suceso!")

def buscarPorNome(event):
    atualizarTabela()
...
label_busca.grid(row=7, column=0)
txt_busca = Entry(janela, font="Arial 14", width=32)
txt_busca.grid(row=7, column=1, columnspan=3, sticky=W)
txt_busca.bind("<KeyRelease>",buscarPorNome)

janela.geometry("460x450") 
janela.mainloop()